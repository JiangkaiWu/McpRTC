...
/* my code here */
// Esp32Camera.cpp - 实现新函数
bool Esp32Camera::GetJpegFrame(uint8_t** out_buf, size_t* out_len) {
    if (fb_ != nullptr) {
        esp_camera_fb_return(fb_);
        fb_ = nullptr;
    }
    ...
    // 如果帧格式不是JPEG，需要进行转换
    // 很多ESP32-CAM配置可以直接输出JPEG，这样效率最高
    if (fb_->format != PIXFORMAT_JPEG) {
        // ESP_LOGI(TAG, "Converting frame from %d to JPEG", fb_->format);
        // frame2jpg 是esp32-camera驱动提供的函数
        bool success = frame2jpg(fb_, 20, out_buf, out_len); // 40是JPEG质量
        if (!success) {
        ...
        // *out_buf = fb_->buf;
        // *out_len = fb_->len;
        // 注意：这种情况下，fb_不能在这里释放，调用者使用完数据后需要手动释放
        // 为了统一，更好的做法是拷贝一份数据
        uint8_t* jpeg_copy = (uint8_t*)malloc(fb_->len);
        memcpy(jpeg_copy, fb_->buf, fb_->len);
        *out_buf = jpeg_copy;
        *out_len = fb_->len;
        esp_camera_fb_return(fb_);
        fb_ = nullptr;

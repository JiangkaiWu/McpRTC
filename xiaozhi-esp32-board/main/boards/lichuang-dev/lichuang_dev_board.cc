...
public:
    ...
    void SetVideoSource(){
        ...
        // 外部时钟 (XCLK) 频率
        dvp_config.xclk_freq = XCLK_FREQ_HZ; 
        dvp_config.i2c_port = 1;

        // 使用新的配置来创建DVP视频源
        webrtc_helper.video_source_ = esp_capture_new_video_dvp_src(&dvp_config);
        
        if (webrtc_helper.video_source_ == NULL) {
            ESP_LOGW(TAG,"video_source_ set failed!");
            return;
        }
        ESP_LOGI(TAG,"video_source_ set success!");

    }
    void SetAudioSource(){
        esp_capture_audio_dev_src_cfg_t codec_cfg ={
            .record_handle = ((BoxAudioCodec*)GetAudioCodec())->GetInputDevice()
        };
        webrtc_helper.audio_source_ = esp_capture_new_audio_dev_src(&codec_cfg);
        if (webrtc_helper.audio_source_ == NULL) {
            ESP_LOGW(TAG,"audio_source_ set failed!");
        }
        ESP_LOGI(TAG,"audio_source_ set success!");
    }
    void SetCaptureHandle(){
        esp_capture_cfg_t cfg = {
            .sync_mode = ESP_CAPTURE_SYNC_MODE_AUDIO,
            .audio_src = webrtc_helper.audio_source_,
            .video_src = webrtc_helper.video_source_,
        };
        esp_capture_open(&cfg, &webrtc_helper.capture_handle_);

        
        ESP_LOGI(TAG,"capture_handle_ set success!");
    }
    virtual webrtcHelper* GetWebRtcHelper() override {
        return &webrtc_helper;
    }

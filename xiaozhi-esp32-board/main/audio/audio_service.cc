...
void AudioService::AudioOutputTask() {
    while (true) {
        ...
        // 检查队列在取出最后一个元素后是否为空
        bool was_last_task = audio_playback_queue_.empty();
        ...
        // ESP_LOGI(TAG, "Audio output task started, is_fist_chunk_of_the_response: %d", is_first_chunk_of_the_response);
        if (is_first_chunk_of_the_response) { // 您需要自己实现这个逻辑判断
            ESP_LOGI(TAG, "First chunk of the response");
            struct timeval tv;
            gettimeofday(&tv, NULL);
            uint64_t current_time = (uint64_t)tv.tv_sec * 1000000 + (uint64_t)tv.tv_usec;
            if (on_playback_start_callback_) {
                ESP_LOGI(TAG, "register success!!");
                on_playback_start_callback_(current_time);
            }
            is_first_chunk_of_the_response = false;
        }
        ...
        if(was_last_task) {
            ESP_LOGI(TAG, "Audio output task finished,reset status");
            is_first_chunk_of_the_response = true;
        }
...
void AudioService::OpusCodecTask() {
    while (true) {
        std::unique_lock<std::mutex> lock(audio_queue_mutex_);
        // ESP_LOGI(TAG,"audio_playback_queue_.size() is %d,audio_decode_queue.size_() is %d",audio_playback_queue_.size(),audio_decode_queue_.size());
        ...
        if (!audio_decode_queue_.empty() && audio_playback_queue_.size() < MAX_PLAYBACK_TASKS_IN_QUEUE) {
            // ESP_LOGI(TAG,"Start decoding,audio_decode_queue_.size() is %d,audio_playback_queue_.size() is %d",audio_decode_queue_.size(),audio_playback_queue_.size());
...
bool AudioService::PushPacketToDecodeQueue(std::unique_ptr<AudioStreamPacket> packet, bool wait) {
    std::unique_lock<std::mutex> lock(audio_queue_mutex_);
    // ESP_LOGI(TAG,"audio_decode_queue_.size()=%d,MAX_DECODE_PACKETS_IN_QUEUE=%d",audio_decode_queue_.size(),MAX_DECODE_PACKETS_IN_QUEUE);
...
void AudioService::PlaySound(const std::string_view& ogg) {
    ...
    while (true) {
        ...
        while (seg_idx < page_segments) {
            ...
            ESP_LOGI(TAG,"push packet to decode queue!, size=%d,decode_queue.size is %d",pkt_len,audio_decode_queue_.size());

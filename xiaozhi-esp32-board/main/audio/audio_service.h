...
public:
    ...
    // 1. 定义一个回调函数的类型别名
    // 这个函数接受一个64位整数（时间戳），没有返回值
    using TimestampCallback = std::function<void(uint64_t)>;

    // 2. 添加一个公共方法，用于从外部注册回调函数
    void registerPlaybackStartCallback(TimestampCallback callback) {
        on_playback_start_callback_ = callback;
    }

private:
    // 3. 添加一个成员变量来存储注册的回调函数
    TimestampCallback on_playback_start_callback_;
    bool is_first_chunk_of_the_response = true;

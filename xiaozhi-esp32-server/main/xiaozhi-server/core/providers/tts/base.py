...
from core.handle.ttsQueueHandle import TimestampTTSQueueHandle
...
class TTSProviderBase(ABC):
    def __init__(self, config, delete_audio_file):
        ...
        self.tts_audio_queue = TimestampTTSQueueHandle()
        ...
        while not self.conn.stop_event.is_set():
            try:
                ...
                if message.sentence_type == SentenceType.FIRST:
                    ...
                    self.is_first_sentence = True
                    self.tts_audio_first_sentence = True
                    self.conn.logger.bind(tag=TAG).info(f"{time.time_ns()}，开始处理TTS文本")
                        ...
                        if self.delete_audio_file:
                            ...
                            if tts_file:
                                ...
                                self.conn.logger.bind(tag=TAG).info(
                                    f"TTS音频入队，队列长度为{self.tts_audio_queue.qsize()}"
                                )
                ...
                if message.sentence_type == SentenceType.LAST:
                    ...
                    self.conn.logger.bind(tag=TAG).info(f"{time.time_ns()}，TTS文本处理结束")
            except queue.Empty:
                continue
            except Exception as e:
                logger.bind(tag=TAG).error(
    ...
    def _audio_play_priority_thread(self):
        while not self.conn.stop_event.is_set():
            ...
            try:
                try:
                    sentence_type, audio_datas, text, timestamp = self.tts_audio_queue.get(
                        timeout=1
                    )
                    # queue_waiting_time = time.perf_counter_ns() - timestamp
                    # self.conn.downqueue_analyzer.update(queue_waiting_time // 1000)
                    # self.conn.logger.bind(tag=TAG).info(
                    #     f"{time.time_ns()}，tts音频出队,时间戳为{timestamp},等待时间为{queue_waiting_time // 1000}，当前队列深度为{self.tts_audio_queue.qsize()}"
                    # )
                    # self.conn.total_logger.log("tts_audio_queue_waiting_time", queue_waiting_time // 1000)
                except queue.Empty:

...
class AppRTCClient:
    ...
    """
    def __init__(self, need_close = 0):
        self.need_close = need_close
        self.room_id = "389298502"
        self.client_id = None
        ...
        self.record_dir = r"E:\xiaozhi\xiaozhi-esp32-server\main\xiaozhi-server\received_images"
        os.makedirs(self.record_dir, exist_ok=True)

        # self.video_frame_queue = video_frame_queue
        self.have_video_track = False
        self.command_queue = asyncio.Queue()  # 用于线程间通信
        
    def keyboard_listener(self):
        """在单独的线程中监听键盘输入"""
        while True:
            try:
                command = input("输入命令 (quit/snapshot/record): ").strip().lower()
                # 使用线程安全的方式将命令放入队列
    ...
    async def process_commands(self):
        ...
        while True:
            ...
            if command == "quit":
                ...
                ringing_msg = {
                "cmd": "send",
                "msg": json.dumps({"type": "customized", "data": "change_fps"}),
                }
                await self.websocket.send(json.dumps(ringing_msg))
                logger.info("sending change fps message.")
            else:

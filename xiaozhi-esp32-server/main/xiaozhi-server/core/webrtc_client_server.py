...
class AppRTCClient:
    ...
    """
    def __init__(self, video_frame_queue: asyncio.Queue , conn, need_close = 0):
        self.need_close = need_close
        self.room_id = "355783181"
        self.client_id = None
        ...
        self.record_dir = os.path.join(base_dir, "received_images")
        os.makedirs(self.record_dir, exist_ok=True)

        self.video_frame_queue = video_frame_queue
        self.conn = conn
        self.have_video_track = False
        conn.video_stream_active = True             # 在哪里设置好，这个需要考虑一下，姑且先放这里
    ...
    def on_track(self,track):
        print(f"[APPRTCClient] 收到轨道: {track.kind}")
        if track.kind == "video":
            print("[AppRTCClient]接收到videotrack，启动接收器任务")
            # 启动专门的协程来处理这个轨道
            asyncio.create_task(self._video_frame_receiver(track))
        elif track.kind == "audio":
            print("[AppRTCClient]接收到audiotrack，暂不处理")
            # 理论上ESP32端不会发送音频过来
            # asyncio.create_task(self._audio_frame_receiver(track))
            pass

    async def _video_frame_receiver(self, track):
        """
        异步接收视频帧，并根据 conn.client_have_voice 自动 start/stop 录制。
        """
        print("[AppRTCClient] 视频帧接收器任务已启动。")
        self.have_video_track = True
        while True:
            try:
                frame = await track.recv()
                timestamp = time.monotonic()

                # 将帧放入队列给 Connection 使用
                await self.video_frame_queue.put((frame, timestamp))

                # -------- 自动 start/stop MediaRecorder --------
                if self.conn.client_have_voice and not self.recording_active:
                    await self._start_recorder(track)

            except Exception as e:
                print(f"[AppRTCClient] 视频接收器任务出错: {e}")
                break
        print("[AppRTCClient] 视频帧接收器任务已停止。")

    async def _start_recorder(self, track):
        """
        启动 MediaRecorder
        """
        filename = f"video_{int(time.monotonic() * 1000)}.mp4"
        ...
        self.have_video_track = True
        print(f"[AppRTCClient] MediaRecorder 已启动: {filepath}")
        # 保存路径到 conn，用于后续上传
        self.conn.current_video_file = filepath
    ...
    async def _stop_recorder(self):
        ...
        if self.recorder:
            ...
            print(f"[AppRTCClient] MediaRecorder 已停止: {self.conn.current_video_file}")
            self.recorder = None
            self.recording_active = False

    # 这一段是debug时的逻辑，把接收到的视频帧保存在本地文件中，现已不再使用
    # def on_track(self, track):
